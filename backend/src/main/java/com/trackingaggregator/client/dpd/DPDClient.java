...
public class DPDClient implements CourierClient {
    ...
    @RestClient
    DPDRestClient restClient;

    @ConfigProperty(name = "courier.dpd.enabled", defaultValue = "false")
    boolean enabled;

    @Inject
    DPDResponseMapper mapper;
    ...
    public Optional<TrackingResult> track(String trackingNumber) {
        try {
            DPDTrackingResponse response = restClient.trackShipment(trackingNumber);
